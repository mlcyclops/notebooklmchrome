# ADR-0010: Harden the experimental chat/generate automation against UI changes
...
1. **Centralized selector config.** A single `AUTOMATION_SELECTORS` object near the
   ...
   `chatResponse`, `guideButton`, `studioOutput`) to an **ordered array of
...
2. **Prioritized multi-strategy resolution.** A `resolveElement(targetKey, root)`
   ...
   `resolveAllElements` returns the de-duped set where the newest of several is
   needed.
...
3. **MutationObserver-based waiting.** `waitForElement(predicateOrTargetKey,
   ...
   onProgress })` observes a streaming response and resolves with the final text
   once it has been unchanged for `quietMs` (or on timeout), replacing the
   arbitrary "25 stale ticks" heuristic with an adaptive quiet-period detector and
   forwarding incremental text via `onProgress`.
...
4. **Graceful, non-throwing failure.** Every unresolved target returns a clear
   ...
   AUTOMATION_SELECTORS.sendButton in content.js"). No exception reaches the page,
   and the existing message shapes the background/server expect (`chat_chunk` with
...
- **Record/replay (e.g. captured Playwright traces):** brittle in the same way as
  hard-coded selectors, adds heavy tooling/deps, and cannot run inside the page's
...
- Supersedes: none. Builds on
  [ADR-0008](0008-premium-ui-ux-and-trustworthy-notebook-states.md) (the listing
  RPC path).
- Superseded by: —
