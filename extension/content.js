...
// -------------------------------------------------------------
// AUTOMATION RESILIENCE LAYER (ADR-0009)
//
...
// timeout. Never rejects — callers branch on a null result. (ADR-0009)
...
async function handleChatRequest(requestId, data) {
  ...
  try {
    ...
    // waiting (MutationObserver-backed) instead of fixed-count polling. (ADR-0009)
...
async function handleGenerateProduct(requestId, data) {
  ...
  try {
    ...
    // format button may already be visible. (ADR-0009)
    ...
    // and not a "Generating…" placeholder. (ADR-0009)
