...
- ✅ Hardened the experimental chat / generate automation against UI changes: centralized `AUTOMATION_SELECTORS` config (chat input, send button, response container, Guide button, Studio output), a prioritized `resolveElement`/`resolveAllElements` multi-strategy resolver (stable attribute/role/aria first → text/aria match → looser structural fallbacks), MutationObserver-based `waitForElement` (immediate check + observer + timeout, no fixed sleeps) and `waitForStableText` (resolves when a streaming response goes quiet), and graceful non-throwing failures that log calm, actionable warnings and return clear error objects. Stays explicitly experimental/best-effort — live correctness still depends on Google's DOM and isn't headlessly verifiable. The companion-server request/response contract (`chat_chunk`/`chat_error`/`response`) is unchanged (ADR-0009).
...
### 2026-06-26 — Harden chat/generate automation against UI changes (ADR-0009)
- Accepted ADR-0009. The chat (`handleChatRequest`) and Studio generate (`handleGenerateProduct`) features drive NotebookLM's obfuscated DOM and stay explicitly experimental/best-effort; this increment makes them break less often and be repairable in one place — it does not promise live reliability (Google's UI can change without notice and live correctness can't be verified headlessly).
- Added a centralized `AUTOMATION_SELECTORS` config near the automation section mapping each logical target (`chatInput`, `sendButton`, `chatResponse`, `guideButton`, `studioOutput`) to an ordered list of candidate strategies — CSS selector strings and/or `{ buttonText }` / `{ text, selector }` text-match descriptors. When Google changes the UI, that one list is the place to edit.
- Added `resolveElement(targetKey, root)` (first matching strategy wins, stable attribute/role/aria first → text/aria match via the existing `findElementByText`/`findButtonByText` → looser structural fallbacks; never throws) and `resolveAllElements` (de-duped, document order — used to grab the newest response bubble).
- Replaced fixed `setTimeout`/fixed-count polling with MutationObserver-based `waitForElement(predicateOrTargetKey, {timeout})` (immediate synchronous check + observer + timeout, resolves the element or `null`, never rejects) and `waitForStableText(el, {quietMs, timeout, onProgress})` (forwards streaming text via `onProgress`, resolves with the final text once it's been unchanged for `quietMs` or on hard timeout).
- Refactored both handlers to resolve/await elements via the config and stream via `waitForStableText`. Every unresolved target now logs a calm, actionable warning ("… NotebookLM UI may have changed; update AUTOMATION_SELECTORS.<key> in content.js") and returns a clear non-throwing error to the server. The companion-server message shapes are preserved verbatim: chat still emits `chat_chunk` `{ text, done }` deltas + a final `done:true`, errors emit `chat_error` `{ error }`, and generate replies with `response` `{ data: { success, content } | { error } }`.
- Tested: `node --check` on content.js/background.js/server.js (pass) + `manifest.json` parses; headless Chromium load via Playwright (global `playwright`, persistent context, `--load-extension`) — MV3 service worker registers, no page/load errors; a hand-stubbed DOM + MutationObserver Node harness (jsdom unavailable) — 13/13 assertions pass (`resolveElement` priority + fallback + unknown-key null; `waitForElement` resolves on mutation, immediate hit, and null-on-timeout; `waitForStableText` resolves after text stabilizes while forwarding progress, and on hard timeout). Live correctness against the real NotebookLM DOM is NOT headlessly verifiable — it remains experimental/best-effort.
- Next: Firefox / Edge packaging.
